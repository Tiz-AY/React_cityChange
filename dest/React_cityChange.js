;(function () {
	var areas = {
		"北京市": {
			"北京市": ["昌平区", "朝阳区", "大兴区", "东城区", "房山区", "丰台区", "海淀区", "怀柔区", "平谷区", "顺义区", "通州区", "西城区", "门头沟区", "石景山区", "密云县", "延庆县"],
			"asg市": ["昌平区", "朝阳区", "大兴区", "东城区", "房山区", "丰台区", "海淀区", "怀柔区", "平谷区", "顺义区", "通州区", "西城区", "门头沟区", "石景山区", "密云县", "延庆县"]
		},
		"天津市": {
			"天津市": ["宝坻区", "北辰区", "东丽区", "汉沽区", "和平区", "河北区", "河东区", "河西区", "红桥区", "津南区", "南开区", "塘沽区", "武清区", "西青区", "蓟县", "静海县", "宁河县", "滨海新区"]
		},
		"河北省": {
			...
			"秦皇岛市": ["海港区", "北戴河区", "山海关区", "昌黎县", "抚宁县", "卢龙县", "青龙满族自治县"],
		...
		"上海市": {
			"上海市": ["宝山区", "奉贤区", "虹口区", "黄浦区", "嘉定区", "金山区", "静安区", "闵行区", "普陀区", "青浦区", "松江区", "徐汇区", "杨浦区", "闸北区", "长宁区", "浦东新区", "崇明县"]
		},
		"江苏省": {
		...
		"浙江省": {
			"杭州市": ["下城区", "上城区", "拱墅区", "西湖区", "江干区", "滨江区", "萧山区", "余杭区", "富阳区", "建德市", "临安市", "桐庐县", "淳安县"],
		...
		"福建省": {
			...
			"龙岩市": ["漳平市", "新罗区", "连城县", "上杭县", "武平县", "永定县", "长汀县"],
		...
		"湖北省": {
			...
			"天门市": ["竟陵街道", "杨林街道", "多宝镇", "多祥镇", "干驿镇", "横林镇", "胡市镇", "黄潭镇", "九真镇", "麻洋镇", "马湾镇", "彭市镇", "石河镇", "拖市镇", "汪场镇", "渔薪镇", "皂市镇", "张港镇", "佛子山镇", "天门侨乡经济技术开发区"],
		...
		"广东省": {
			...
			"深圳市": ["宝安区", "福田区", "龙岗区", "罗湖区", "南山区", "盐田区", "光明新区"],
		...
		"重庆市": {
			"重庆市": ["巴南区", "北碚区", "大足区", "涪陵区", "高新区", "合川区", "江北区", "江津区", "南岸区", "南川区", "綦江区", "黔江区", "双桥区", "万盛区", "万州区", "永川区", "渝北区", "渝中区", "长寿区", "大渡口区", "九龙坡区", "沙坪坝区", "开县", "忠县", "璧山县", "城口县", "垫江县", "丰都县", "奉节县", "梁平县", "荣昌县", "铜梁县", "潼南县", "巫山县", "巫溪县", "武隆县", "云阳县", "石柱土家族自治县", "彭水苗族土家族自治县", "秀山土家族苗族自治县", "酉阳土家族苗族自治县"]
		},
		"四川省": {
		...
		"贵州省": {
			...
			"毕节市": ["七星关区", "大方县", "金沙县", "纳雍县", "黔西县", "织金县", "威宁彝族回族苗族自治县"],
	...
	var ReactCityChange = React.createClass({
		displayName: "ReactCityChange",

		getInitialState: function () {
			var province = new Array();
			var firstProFlag = 0;
			var firstProvinceName;
			for (var keys in areas) {
				if (firstProFlag == 0) {
					firstProvinceName = keys;
					firstProFlag = 1;
				}
				province.push(keys);
			}
			var datas = this.getProvince(areas, firstProvinceName);
			var citys = datas['city'];
			var cityData = datas['cityData'];
			var zones = datas['zone'];
			return {
				'province': province,
				'data': areas,
				'city': citys,
		...
		getProvince: function (areas, province) {
			/*获取省份*/
			var cityData = this.getCity(areas, province);
			var firstCityName,
			    firstCityFlag = 0,
			    citys = new Array();
			for (var keys in cityData) {
				if (firstCityFlag == 0) {
					firstCityName = keys;
					firstCityFlag = 1;
				}
			...
			var zones = this.getZone(cityData, firstCityName);
		...
		getCity: function (areas, province) {
			/*取指定省下的城市*/
			var citys = new Array();
			for (var key in areas) {
				if (key == province) {
					var cData = areas[key];
					for (var cityKey in cData) {
						citys[cityKey] = cData[cityKey];
					}
				}
			}
			return citys;
		},
		getZone: function (citys, firstCityName) {
			/*取指定城市下的区*/
			var zones = new Array();
			for (var key in citys) if (key == firstCityName) zones = citys[key];
			return zones;
		},

		provinceChange: function (_e) {
			var targetProvince = typeof _e == 'string' ? _e : _e.target.value;
			var cityData = this.getCity(this.state.data, targetProvince);
			var firstCityName,
			    firstCityFlag = 0,
			    citys = new Array();
			for (var keys in cityData) {
			...
			var zones = this.getZone(cityData, firstCityName);
			this.setState({ city: citys, cityData: cityData, zone: zones });
		},
		cityChange: function (_e) {
			var targetCity = _e.target.value;
			var zones = this.getZone(this.state.cityData, targetCity);
			this.setState({ zone: zones });
		},
		render: function () {

			var provinces = this.state.province.map(function (p) {
			...
			var city = this.state.city.map(function (c) {
				return React.createElement(
					"option",
					null,
					c
			...
			var zone = this.state.zone.map(function (z) {
				return React.createElement(
					"option",
					null,
					z
	...
	ReactDOM.render(React.createElement(ReactCityChange, null), document.getElementById('container'));
})();